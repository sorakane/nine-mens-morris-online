...
export async function POST(r: Request) {
  try {
    ...
    if (b.action === 'create') {
      ...
      const spectatorsAllowed = b.spectatorsAllowed ?? b.capacity !== 2;
      if (typeof spectatorsAllowed !== 'boolean')
        return reply({ error: '観戦の設定を選んでください' }, 400);
    ...
    if (b.action === 'join') {
      ...
      if (!state.spectatorsAllowed && state.members.length >= 2)
        return reply(
          {
            error:
              'この部屋は対戦者2人専用です。観戦するには、観戦ありの部屋に参加してください。',
          },
          409,
        );
      if (!name) return reply({ error: '名前を入力してください' }, 400);
